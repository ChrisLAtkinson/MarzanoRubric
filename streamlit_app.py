...
st.title("Interactive Head of School Evaluation Rubric")
st.write("""
This tool evaluates Heads of School based on the Marzano School Leadership Evaluation Model.
You can score domains, provide evidence, view detailed rubrics in table format, and save or print the results.
""")
...
for i, item in enumerate(rubric_data):
    ...
    st.write(rubric_df.style.hide_index())  # Hide index to ensure only two columns
...
if st.button("Save Results"):
    ...
    results_df.to_csv('head_of_school_evaluation_results.csv', index=False)
    st.success("Results saved as 'head_of_school_evaluation_results.csv'.")

if st.button("Download Results"):
    st.download_button(
        label="Download CSV",
        data=results_df.to_csv(index=False).encode('utf-8'),
        file_name='head_of_school_evaluation_results.csv',
        mime='text/csv'
    )
